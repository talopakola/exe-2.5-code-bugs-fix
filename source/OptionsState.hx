...
class OptionsState extends MusicBeatState
{
	...
	override function create() {
		...
		menuBG = new FlxSprite().loadGraphic(Paths.image('backgroundlool2'));
...
class PreferencesSubstate extends MusicBeatSubstate
{
		...
		if(usesCheckbox) {
			if(controls.ACCEPT && nextAccept <= 0) {
				switch(options[curSelected]) {
					...
					case 'Disable Timebar And BG':
						ClientPrefs.disableTimeBarAndBG = !ClientPrefs.disableTimeBarAndBG;
	...
	function changeSelection(change:Int = 0)
	{
		...
		switch(options[curSelected]) {
			...
			case 'Disable Timebar And BG':
				daText = "This disable the timebar and timebar bg, but the song text continue";
			case 'Improved Hold Renderer':
		...
		for (i in 0...checkboxArray.length) {
			...
			if(checkbox != null) {
				...
				switch(options[checkboxNumber[i]]) {
					...
					case 'Disable Timebar And BG':
						daValue = ClientPrefs.disableTimeBarAndBG;
					case 'Improved Hold Renderer':
