...
// HIS NAME IS CHAOTIX!!!
// PLS KILL ME AHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH
...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'starved':
				...
				if(!ClientPrefs.gore) {
					pizzaHutStage = new BGSprite('starved/stageng', -100, 0, 1, 0.9); //Starved stage no gore lol
					pizzaHutStage.setGraphicSize(Std.int(pizzaHutStage.width * 1.5));
					add(pizzaHutStage);
				}
				...
				// sonic died
				if(ClientPrefs.gore) {
				deadHedgehog = new BGSprite('starved/sonicisfuckingdead', 0, 100, 1, 0.9); //Oh nooooooooooooooooo sonic is died :(
		...
		{
			case 'fatality' | "milk" | 'round-a-bout' | "sunshine":
		...
		timeBarBG.xAdd = -4;
		timeBarBG.yAdd = -4;
		if (!ClientPrefs.disableTimeBarAndBG) {
		add(timeBarBG);
		}
		...
		if (SONG.isRing)
		{
			keysArray = [
				ClientPrefs.copyKey(ClientPrefs.keyBinds.get('note_left')),
				ClientPrefs.copyKey(ClientPrefs.keyBinds.get('note_down')),
				ClientPrefs.copyKey(ClientPrefs.keyBinds.get('ring')),
				ClientPrefs.copyKey(ClientPrefs.keyBinds.get('note_up')),
				ClientPrefs.copyKey(ClientPrefs.keyBinds.get('note_right'))
			];
		}
	...
	override public function update(elapsed:Float)
	{
		...
		if (SONG.canDodge && FlxG.keys.justPressed.SPACE) {
			dodging = true;
	...
	{ // You didn't hit the key and let it go offscreen, also used by Hurt Notes
		...
		switch (daNote.noteType)
		{
			case "Static Note":
				...
				var daNoteStaticc = new FlxSprite(0, 0).loadGraphic(Paths.image("hitStatic", 'exe'));
				daNoteStaticc.frames = Paths.getSparrowAtlas('hitStatic');
				daNoteStaticc.animation.addByPrefix('static', "staticANIMATION", 24, false);
				daNoteStaticc.animation.play('static');
				daNoteStaticc.cameras = [camHUD];
				add(daNoteStaticc);
				...
				{
					daNoteStaticc.alpha = 0;
					trace('ended HITSTATICLAWL');
					remove(daNoteStaticc);
				});
